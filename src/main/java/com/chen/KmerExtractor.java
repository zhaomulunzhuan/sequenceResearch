...
public class KmerExtractor {//原始数据集是长序列集合，每个原始数据集生成一个对应的kmers集合的数据集文件
    ...
    public static void getKmersFromFile(String filePath, int k, int index) throws IOException {//两个标题行间的序列是一段连续序列，不跨标题行
            ...
            while ((line = reader.readLine()) != null) {
                if (line.startsWith(">")) { // 当前行是标题行
                    if (sequenceBuilder.length() > 0) {
                        // 处理已经读取的序列
                        processSequence(sequenceBuilder.toString(), k, index, filePath);
    ...
    private static void processSequence(String sequence, int k, int index, String filePath) throws IOException {
